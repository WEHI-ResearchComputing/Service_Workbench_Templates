...
###################### WEHI
# A variation on the script sourced and invoked via:
# The UserData section of RStudio/cfn-templates/ec2-rlstudio.yaml
# --> get_bootstrap.sh --> bootstrap.sh --> mount_s3.sh
###################### WEHI
...
# }, ...]
CONFIG="/host/etc/s3-mounts.json"
###CONFIG="/usr/local/etc/s3-mounts.json"
MOUNT_DIR="/home/rstudio/studies"
...
do
    ...
    then
        ...
        then
            ...
            /usr/local/bin/goofys --region $region --acl "bucket-owner-full-control" -o allow_other --file-mode=0777 --dir-mode=0777 "${s3_bucket}:${s3_prefix}" "$study_dir"
            ###goofys --region $region --acl "bucket-owner-full-control" -o allow_other --file-mode=0777 --dir-mode=0777 "${s3_bucket}:${s3_prefix}" "$study_dir"
        else
